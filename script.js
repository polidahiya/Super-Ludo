...
//
function setsteps(dice_outcome) {
    ...
    if (flags.dice_value != 6) {
      if (dice_number != 4) {
        selectall(".dice").forEach((item, index) => {
          index == dice_number
            ? (item.style.border = "3px solid rgb(0, 255, 0)")
...
function player_position_style(value1, value2) {
  ...
  select1("." + value1).style.setProperty(
    "--y",
    `${Math.floor(value2 / 10) * 10}%`
  );
}
...
function check_cut(value) {
  if (value == 1) {
    for (const key in players_position) {
      if (
        players_position.player1 == players_position[key] &&
        key != "player1"
      ) {
  ...
  if (value == 2) {
    for (const key in players_position) {
      if (
        players_position.player2 == players_position[key] &&
        key != "player2"
      ) {
        players_position[key] = 0;
        if (key == "player1") {
  ...
  if (value == 3) {
    for (const key in players_position) {
      if (
        players_position.player3 == players_position[key] &&
        key != "player3"
      ) {
        players_position[key] = 0;
        if (key == "player2") {
  ...
  if (value == 4) {
    for (const key in players_position) {
      if (
        players_position.player4 == players_position[key] &&
        key != "player4"
      ) {
        players_position[key] = 0;
        if (key == "player2") {
...
function checkspecialtile(value) {
  setTimeout(() => {
    setsteps(20);
    if (players_position[value] == 7 && teleport[value]) {
      players_position[value] = 19;
      player_position_style(value, players_position[value]);
      teleport[value] = false;
    }
    if (players_position[value] == 19 && teleport[value]) {
      players_position[value] = 43;
      player_position_style(value, players_position[value]);
      teleport[value] = false;
    }
    if (players_position[value] == 43 && teleport[value]) {
      players_position[value] = 68;
      player_position_style(value, players_position[value]);
      teleport[value] = false;
    }
    if (players_position[value] == 68 && teleport[value]) {
      players_position[value] = 84;
      player_position_style(value, players_position[value]);
      teleport[value] = false;
    }

    if (players_position[value] == 27 && danger[value]) {
      players_position[value] = 10;
      player_position_style(value, players_position[value]);
      danger[value] = false;
    }
    if (players_position[value] == 32 && danger[value]) {
      players_position[value] = 27;
      player_position_style(value, players_position[value]);
      danger[value] = false;
    }
    if (players_position[value] == 71 && danger[value]) {
      players_position[value] = 32;
      player_position_style(value, players_position[value]);
      danger[value] = false;
    }
    if (players_position[value] == 98 && danger[value]) {
      players_position[value] = 71;
      player_position_style(value, players_position[value]);
      danger[value] = false;
    }
  }, 2000);
